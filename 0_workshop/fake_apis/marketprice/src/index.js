...
const getMarketPrice = (req, res) => {
  ...
  if (ingredient === undefined) {
    res.statusCode = 400;
    res.end('missing ingredient parameter');
  }

  connectToMongo().catch(reason => {
    ...
    collection.find({ 'ingredient': ingredient }).toArray()
      .then(result => {
        if (result === null) {
          res.end({});
        } else {
...
module.exports = (req, res) => { getMarketPrice(req, res) };

if (!process.env.IS_NOW) {
  const http = require('http');
  const app = http.createServer((req, res) => { getMarketPrice(req, res) });
